module can_apb_slave #(
     ...
     *
     * Split into independent conditions so that illegal_write
     * has only one driver.
     * ============================================================ */

    wire illegal_write_readonly;
    wire illegal_write_timing;
    wire illegal_write_control;
    wire illegal_write;

    /*
     * Read-only register writes.
     */
    assign illegal_write_readonly =
        apb_write &&
        (
            (PADDR == ADDR_VERSION) ||
            (PADDR == ADDR_STATUS) ||
            (PADDR == ADDR_ERROR_STATUS) ||
            (PADDR == ADDR_TX_STATUS) ||
            (PADDR == ADDR_RX_STATUS) ||
            (PADDR == ADDR_RX_ID) ||
            (PADDR == ADDR_RX_CTRL) ||
            (PADDR == ADDR_RX_DATA_LO) ||
            (PADDR == ADDR_RX_DATA_HI) ||
            (PADDR == ADDR_TEC) ||
            (PADDR == ADDR_REC)
        );

    /*
     * Timing configuration and acceptance filter writes are
     * illegal while CAN is enabled.
     */
    assign illegal_write_timing =
        apb_write &&
        can_enable &&
        (
            (PADDR == ADDR_BRP) ||
            (PADDR == ADDR_SEGMENTS) ||
            (PADDR == ADDR_SJW_CONTROL) ||
            (PADDR == ADDR_FILTER0_ID) ||
            (PADDR == ADDR_FILTER0_MASK) ||
            (PADDR == ADDR_FILTER0_CTRL) ||
            (PADDR == ADDR_FILTER1_ID) ||
            (PADDR == ADDR_FILTER1_MASK) ||
            (PADDR == ADDR_FILTER1_CTRL)
        );

    /*
     * CONTROL register restrictions.
     *
     * Clearing CAN_ENABLE while TX is active is illegal.
     *
     * LOOPBACK + LISTEN_ONLY is illegal.
     */
    assign illegal_write_control =
        apb_write &&
        (PADDR == ADDR_CONTROL) &&
        (
            (can_enable && !PWDATA[0] && tx_busy) ||
            (PWDATA[1] && PWDATA[2])
        );

    /*
     * Single driver for illegal_write.
     */
    assign illegal_write =
        illegal_write_readonly ||
        illegal_write_timing ||
        illegal_write_control;
