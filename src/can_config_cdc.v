 ...
 *   FILTER1_IDE
 *
 * The payload is captured in the PCLK domain and held stable while
 * the request is pending.
 *
 * A request toggle crosses from PCLK to CAN_CLK through a 2-FF
 * synchronizer.
 *
 * The CAN domain captures the complete snapshot when the request
 * toggle changes.
 *
 * An acknowledgement toggle returns from CAN_CLK to PCLK through
 * another 2-FF synchronizer.
 *
 * IMPORTANT:
 *
 * The payload buses themselves are NOT individually synchronized.
 * They are protected by the handshake protocol:
 *
 *   1. Capture payload in PCLK domain.
 *   2. Change request toggle.
 *   3. Hold payload stable while transaction is pending.
 *   4. CAN domain detects synchronized request.
 *   5. CAN domain captures complete payload.
 *   6. CAN domain changes acknowledgement toggle.
 *   7. PCLK domain sees acknowledgement and releases pending state.
 *
 * This is appropriate because configuration changes are infrequent
 * and the payload remains stable for the entire CDC transaction.
 *
 * ================================================================
 */
...
module can_config_cdc (
    ...
    begin
        ...
        begin
            ...
            /*
             * The CAN side has consumed the current configuration.
             */
            if(p_config_pending &&
             ...
             *
             * Capture every configuration signal before changing
             * the request toggle.
             */
